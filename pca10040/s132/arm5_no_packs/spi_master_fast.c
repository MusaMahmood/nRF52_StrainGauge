...
void spi_set_frequency(SPI_config_t *spi_config) {
//disable
  spi_base[0]->ENABLE = (SPI_ENABLE_ENABLE_Disabled << SPI_ENABLE_ENABLE_Pos);

  nrf_gpio_pin_set(spi_config->pin_CSN);
  spi_base[0]->FREQUENCY = (uint32_t)spi_config->frequency << 24;
  spi_base[0]->CONFIG = spi_config->config.SPI_cfg;

  spi_base[0]->EVENTS_READY = 0;
  /* Enable */
  spi_base[0]->ENABLE = (SPI_ENABLE_ENABLE_Enabled << SPI_ENABLE_ENABLE_Pos);
}
